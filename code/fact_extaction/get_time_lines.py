...
def get_target_nodes(codes,verison_num,is_buggy=False,compared_version=None):
    ...
    path=os.path.join(compiled_path,verison_num,'ast','standard')
    file=os.listdir(path)[0]
    path=os.path.join(compiled_path,verison_num,'ast','standard',file)
    ...
    cmp_path=os.path.join(compiled_path,compared_version,'ast','standard')
    cmp_file_name=os.listdir(cmp_path)[0]
    cmp_path=os.path.join(compiled_path,compared_version,'ast','standard',cmp_file_name)
...
def judge_pairs(nodes_old,nodes_new,add_codes,del_codes,code_file_new,code_file_old,entry_function_old,entry_function_new):
    ...
    return False
def main():
    codes=get_codes('./ZepCompareCode/release4.json')
    res=[]
    all_versions=set()
    for version in version_pairs:
        all_versions.add(version)
        all_versions.add(version_pairs[version])
    all_version_codes={}
    for verison_num in all_versions:
        verison=get_solc_version(verison_num)
        or_source_path=os.path.join(source_path,verison)
        with open(or_source_path, 'rb') as file:
            code_file = file.read()
        code_file=code_file.decode('utf-8')
        code_file=code_file.split('\n')
        code_file=[x.strip(" ") for x in code_file]
        code_file='\n'.join(code_file)
        all_version_codes[verison_num]=code_file
    min_version={}
    changed_code={}
    min_version2={}

    for new_version in tqdm(version_pairs):
        # if version!='3.0.0':
        #     continue
        # print('Processing version:',version)
        old_version=version_pairs[new_version]
        ...
        for desc in (target_nodes):
            for key in target_nodes[desc]:
                str_key=f'{new_version.replace("-solc-0.7","")}_{key}'
                if str_key not in min_version:
                    min_version[str_key]=set()
                if str_key not in changed_code:
                    changed_code[str_key]=[]
                nodes_old=target_nodes[desc][key]['old']
                for node in nodes_old:
                    node_code=get_code_line(node,code_file_old)
                    node_code='\n'.join(node_code)
                    changed_code[str_key].append(node_code)
                    for version in all_version_codes:
                        if node_code in all_version_codes[version]:
                            min_version[str_key].add(version.replace('-solc-0.7',''))
        for key in min_version:
            min_version[key]=list(min_version[key])
            min_version[key].sort(key=lambda x: int(x.split('.')[0])*10000+int(x.split('.')[1])*100+int(x.split('.')[2]))

        for desc in (target_nodes):
            for key in target_nodes[desc]:
                str_key=f'{new_version.replace("-solc-0.7","")}_{key}'

                if str_key not in min_version2:
                    min_version2[str_key]=set()
                nodes_old=target_nodes[desc][key]['old']
                flag=True
                for version in all_version_codes:
                    for node in nodes_old:
                        node_code=get_code_line(node,code_file_old)
                        node_code='\n'.join(node_code)
                        if node_code not in all_version_codes[version]:
                            flag=False
                            break
                    if flag:
                        min_version2[str_key].add(version.replace('-solc-0.7',''))
        for key in min_version2:
            min_version2[key]=list(min_version2[key])
            min_version2[key].sort(key=lambda x: int(x.split('.')[0])*10000+int(x.split('.')[1])*100+int(x.split('.')[2]))
    with open('min_version.json','w') as f:
        json.dump(min_version,f)
    with open('changed_code.json','w') as f:
        json.dump(changed_code,f)
    for key in min_version:
        print(f'{key} :::: {len(min_version[key])}')

    print('---------------------------------')
    with open('min_version1.json','w') as f:
        json.dump(min_version2,f)
    for key in min_version2:
        print(f'{key} :::: {len(min_version2[key])}')
