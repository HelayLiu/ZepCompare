...
// BDepToken with Governance.
contract BDepToken is ERC20, AccessControl {

    // Create a new role identifier for the minter role
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor() public ERC20("Depth Token", "BDEP") {
    ...
    /// @notice Creates `_amount` token to `_to`.Must only be called by the self.
    function burn(address _to, uint256 _amount) public {

        // Check that the calling account has the minter role
        require(_to==msg.sender, "Caller is not self");
