...
contract PDEX is ERC20, Ownable {
    ...
    address public empireWmaticPair;
    uint256 public end;
    ...
    uint256 private constant TEAM_PERCENTAGE = 0.05 ether; // 5%
    uint256 private constant OTHER_PERCENTAGE = 0.05 ether; // 5% 
    ...
    modifier onlyPair() {
        require(
            msg.sender == empireWmaticPair,
            "Empire::onlyPair: Insufficient Privileges"
        );
        _;
    }

    constructor() public ERC20("EmpireDEX Polygon", "PDEX") Ownable() {
    }
    ...
    function beginLGE(IEmpireFactory _factory) external onlyOwner() {
        ...
        empireWmaticPair = _factory.createPair(WMATIC, address(this), pairType, 0);
    }
    ...
    function complete() external onlyOwner() {
        ...
        uint256 teamAllocation = _totalRaised.wmul(TEAM_PERCENTAGE); // 5%
        uint256 otherAllocation = _totalRaised.wmul(OTHER_PERCENTAGE); // 5%
        ...
        IWMATIC(WMATIC).deposit{value: _totalRaised}();

        _mint(empireWmaticPair, 5000 * 1 ether); //  50% / 5000 for LGE

        IERC20(WMATIC).safeTransfer(empireWmaticPair, _totalRaised);

        IEmpirePair(empireWmaticPair).mint(address(this));

        _mint(address(this), 250 * 1 ether); // 2.5% / 250 bonus EMPIRE tokens for contributors

        _mint(empireTeam, 2000 * 1 ether); // 20% / 2000 for the EMPIRE team
    ...
    function claim() external {
        ...
        IERC20(empireWmaticPair).safeTransfer(
            msg.sender,
            IERC20(empireWmaticPair)
    ...
    function sweep(uint256 amount, bytes calldata data) external onlyOwner() {
        IEmpirePair(empireWmaticPair).sweep(amount, data);
    }

    function empireSweepCall(uint256 amount, bytes calldata) external onlyPair() {
        IERC20(WMATIC).safeTransfer(owner(), amount);
    }

    function unsweep(uint256 amount) external onlyOwner() {
        IERC20(WMATIC).approve(empireWmaticPair, amount);
        IEmpirePair(empireWmaticPair).unsweep(amount);
    }
    ...
    function transferFrom(
        ...
        uint256 burned;
        if (from != empireWmaticPair && to != empireWmaticPair) {
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        if (msg.sender != empireWmaticPair && to != empireWmaticPair) {
    ...
    function changePair(address newPair) external onlyOwner {
        require(newPair != address(0), "Cannot add burn address");
        empireWmaticPair = newPair;
    }
