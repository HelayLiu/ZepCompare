...
from utils.code_fact import functionFacts, singleFact,factsType, binaryOperationFacts
...
def get_node_facts(function:FunctionContract,node_list: list[Node],var_dic:dict,code_file='',funtion_sig=None)->list[singleFact]:
    ...
    return result
# def get_function_facts(function:FunctionContract,var_dic:dict)->functionFacts:
#     stmts=find_target_statement(function)
#     result=functionFacts()
#     result.function_signature.get_signature_in_func(function)
...
def process_node_sub(node,code_file):
    ...
    if node_type=='IF_LOOP' or node_type=='IFLOOP':
        node_type='IF'
    elif node_type=='OTHER_ENTRYPOINT' or node_type.startswith('END') or  node_type.startswith('START') or node_type=='PLACEHOLDER':
        return None
        ...
        if get_callee_inter:
            for node in called.nodes:
                ...
                if node_process!=None:
                    if isinstance(node_process,singleFact):
                        function_content.append(node_process)
                    else:
                        function_content.extend(node_process)
